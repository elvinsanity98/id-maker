...
import { listOrgs } from "@/lib/orgSettings";
...
export default function AttendanceReport() {
  ...
  const [event, setEvent] = useState<AttendanceFilter["event"]>("");
  ...
  const run = useCallback(async () => {
    setLoading(true);
    setError(null);
    const { data, error: err } = await queryAttendance({ from, to, status, event, org, search });
    setLoading(false);
    if (err) {
      setError(err);
      return;
    ...
    setRows(data);
  }, [from, to, status, event, org, search]);
  ...
  const counts = useMemo(() => {
    ...
    let late = 0;
    rows.forEach((r) => {
      if (r.status === "on-time" || r.status === "present") onTime++;
  ...
  const exportCsv = () => {
    ...
    downloadCsv(`attendance-${org || "all"}-${today()}.csv`, attendanceToCsv(rows));
  };

  return (
    <section className="bg-white rounded-xl shadow-sm p-5">
      <div className="no-print grid gap-3 sm:grid-cols-2 lg:grid-cols-3 mb-4">
        ...
        <label className="text-xs font-medium text-slate-600">
          ...
          <select
            value={event}
            onChange={(e) => setEvent(e.target.value as AttendanceFilter["event"])}
            className="w-full mt-1 px-3 py-2 border border-slate-300 rounded-md text-sm bg-white text-slate-900"
          >
      ...
      <div className="flex flex-wrap gap-3 mb-4 text-sm">
        ...
        <Pill label="On-time" value={counts.onTime} cls="bg-emerald-100 text-emerald-800" />
        <Pill label="Late" value={counts.late} cls="bg-red-100 text-red-800" />
      </div>
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {rows.map((r) => {
                ...
                const onTime = r.status === "on-time" || r.status === "present";
                return (
                  <tr key={r.id} className="border-b border-slate-100">
                    ...
                    <td className="py-1.5 pr-3 text-slate-600 truncate max-w-[10rem]">{r.org ?? "—"}</td>
                    <td className="py-1.5 pr-3 text-slate-600">{r.event === "out" ? "Time Out" : "Time In"}</td>
                    <td className="py-1.5 pr-3">
                      <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded ${
                        onTime ? "bg-emerald-100 text-emerald-800" : "bg-red-100 text-red-800"
                      }`}>
                        {r.status}
                      </span>
                    </td>
